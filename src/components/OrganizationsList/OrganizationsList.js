			...
			<div>
				<div className="sort-bar">
					<ul className="sort-options-list">
						...
						<li className={classNames(["sort-by-amount sort-options-item", {active: sortBy == 'byAmount'}])} onClick={() => {this.changeSortCriteria('byAmount')}}>Monto Total <i className="tooltip-info">En la información que disponemos, los contratos ganados por un consorcio no especifican cómo se divide el monto entre las distintas empresas. Por esta razón, solo aparece el monto total en el que participan las empresas aunque éstas no reciban la totalidad del importe.</i><span className="arrows"><i className={classNames(["arrow-top", {active: reverse && sortBy == 'byAmount'}])}>▲</i><i className={classNames(["arrow-bottom", {active: !reverse && sortBy == 'byAmount'}])}>▼</i></span></li>
					</ul>
				</div>
				{
					contractsLoaded ?
